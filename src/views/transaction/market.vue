<template>
  <div class="layout">
    <div class="filter-container">
      ...
      <div style="float:right;display:flex;justify-content: center;align-items: center;height:40px">
      	<span style="margin-right:20px;margin-top:4px">机器人</span>
      	<el-switch v-model='checkbox' @change='change'></el-switch>
      </div>
...
<el-table-column align="center" label="市场标题" width="120">
	<template slot-scope="scope">
					  <span>{{scope.row.basisV +'_' + scope.row.shortName}}</span>
					</template>
</el-table-column>
<!--
...
-->
<el-table-column align="center" label="买单交易最小数量">
	<template slot-scope="scope">
					  <span>{{scope.row.buyMinTradeSum}}</span>
					</template>
</el-table-column>
<el-table-column align="center" label="卖单交易最小数量">
	<template slot-scope="scope">
					  <span>{{scope.row.sellMinTradeSum}}</span>
					</template>
</el-table-column>


<el-table-column align="center" label="最小买入价">
	<template slot-scope="scope">
					  <span>{{scope.row.minBuyPrize}}</span>
					</template>
</el-table-column>
<el-table-column align="center" label="最大买入价">
	<template slot-scope="scope">
					  <span>{{scope.row.maxBuyPrize}}</span>
					</template>
</el-table-column>
<el-table-column align="center" label="最小买出价">
	<template slot-scope="scope">
					  <span>{{scope.row.minSellPrize}}</span>
					</template>
</el-table-column>
<el-table-column align="center" label="最大买出价">
	<template slot-scope="scope">
					  <span>{{scope.row.maxSellPrize}}</span>
					</template>
</el-table-column>
...
<el-dialog :title="title" :visible.sync="show_dialog">
	<el-form ref="dataForm" label-position="left" label-width="140px" style='width: 400px; margin-left:50px;'>
		<el-form-item label="币种id" prop="type" v-show='false'>
			<el-input v-model='form.currId'></el-input>
		</el-form-item>
		<el-form-item label="基础市场id" prop="type" v-show='false'>
			<el-input v-model='form.basisId'></el-input>
		</el-form-item>
		<el-form-item label="最新价格" prop="type" v-show='false'>
		...
		<el-form-item label="最小卖出价" prop="type">
			<el-input v-model='form.minSellPrize'></el-input>
		</el-form-item>
		<el-form-item label="最大买入价" prop="type">
			<el-input v-model='form.maxBuyPrize'></el-input>
		</el-form-item>
		<el-form-item label="最小买入价" prop="type">
			<el-input v-model='form.minBuyPrize'></el-input>
		</el-form-item>
		<el-form-item label="昨天收盘价" prop="type" v-show='false'>
			<el-input v-model='form.yesterdayShou'></el-input>
		</el-form-item>
		<el-form-item label="今日开盘价" prop="type" v-show='false'>
		...
		<el-form-item label="买家赠送是否开启" prop="type">
			<el-select v-model='form.buySendState' style='width:100%'>
				<el-option label="开启" value="0"></el-option>
				<el-option label="关闭" value="1"></el-option>
			</el-select>
		</el-form-item>
		<el-form-item label="卖家赠送是否开启" prop="type">
			<el-select v-model="form.sellSendState" style='width:100%'>
				<el-option label="开启" value="0"></el-option>
				<el-option label="关闭" value="1"></el-option>
			</el-select>
		</el-form-item>
		<el-form-item label="买家赠送币种" prop="type">
			<el-select v-model="form.buySendCurrId" style='width:100%'>
				<el-option :label="item.shortName" :value="item.id" v-for='item in options1'></el-option>
			</el-select>
		</el-form-item>
		<el-form-item label="卖家赠送币种" prop="type">
			<el-select v-model="form.sellSendCurrId" style='width:100%'>
				<el-option :label="item.shortName" :value="item.id" v-for='item in options1'></el-option>
			</el-select>
		</el-form-item>
				...
				if (this.type == 'add') {
					Post({
						...
						data: {
							...
							minSellPrize: this.form.minSellPrize,
							maxBuyPrize: this.form.maxBuyPrize,
							minBuyPrize: this.form.minBuyPrize,
