...
<el-table-column align="center" label="用户开通币种个数">
    <template slot-scope="scope">
					  <span>{{scope.row.uservircount}}</span>
					</template>
</el-table-column>
...
<el-dialog :title="title" :visible.sync="show_dialog">
    <el-table :key='tableKey' :data="desc_list" border fit highlight-current-row style="width: 100%;margin-top:20px">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            edit_item(row) {
                Get({
                    url: 'virtualWallet/findAllByWallet',
                    data: {
                        userId: row.userId
                    },
                    success: res => {
