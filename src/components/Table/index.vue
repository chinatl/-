...
<el-dialog :title="title" :visible.sync="show_dialog">
    <el-form ref="dataForm" label-position="right" label-width="160px" style='width: 400px; margin-left:50px;'>
        <el-form-item prop="type" v-for='item in form_data' :label="item.label+'：'">
            <el-input v-model='form[item.prop]' v-if='item.type === undefined' :placeholder='form[item.placeholder]'></el-input>
            <el-input type="textarea" v-model="form.desc" v-if='item.type === "textarea"' :placeholder='form[item.placeholder]'></el-input>
            <el-select v-model="form[item.prop]" placeholder="请选择" value-key="id" v-if='item.type === "select"' style='width:240px' :placeholder='form[item.placeholder]'>
                <el-option v-for="(item,i) in item.options" :key="item.id" :label="item.id" :value="item.value">
                </el-option>
            </el-select>
            <el-date-picker v-if='item.type === "date"' v-model="form[item.prop]" type="date" placeholder="选择日期">
            </el-date-picker>
            <el-upload class="upload-demo" action="https://jsonplaceholder.typicode.com/posts/" :on-preview="handlePreview" :on-remove="handleRemove" :before-remove="beforeRemove" multiple :limit="3" :on-exceed="handleExceed" v-if='item.type === "uplode"'>
